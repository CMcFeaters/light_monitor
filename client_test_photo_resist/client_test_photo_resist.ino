...
const int night_mode=0; //1 puts it in night mode where it sleeps till morning
...
my_wifi set_ip(){
  ...
  if (test_var==1){
     //internal network
     MY_SSID = lib_const::MY_SSID;
     MY_PW = lib_const::MY_PW;
     host="192.168.1.237";
     port = 10000;
     //WiFi.begin(MY_SSID,MY_PW);
     IPAddress my_ip(192,168,1,225);   
     IPAddress my_gateway(192,168,1,11);   
     IPAddress subnet(255,255,255,0);   
     return my_wifi(MY_SSID,MY_PW, host,port,my_ip,my_gateway);
    ...
    host="192.168.0.10";
    port = 10000;
    IPAddress my_ip(192,168,0,14);   
    IPAddress my_gateway(192,168,0,10);   
    IPAddress subnet(255,255,255,0);   
    return my_wifi(MY_SSID,MY_PW, host,port,my_ip,my_gateway);
  }
...
void setup() {
  ...
  //secret sauce for saving battery?
  /*WiFi.mode( WIFI_OFF );
  ...
  if ((!state_3) && ((rtcMem.write_to_RTC_MEM((uint16_t)(analogRead(sensorPin))))||(state_2))){
    Serial.println("State: BROADCAST");
    my_wifi temp_wifi=set_ip();
    Serial.println("IP_SET");
    temp_wifi.connect_to_server();
    if (WiFi.status()==WL_CONNECTED){
      Serial.println("State: SENDING*");
      Serial.println("Connected_to_server");
      temp_wifi.send_data((uint16_t*)&rtcMem.rtcData.data,rtcMem.rtcData.count,sleep_time,night_mode);
    }else{
      Serial.println("Wifi_Failed");
    }
    Serial.println("State: CLEAR_MEM");
  ...
  if (state_1){ 
    ...
    Serial.println("State: YES_RF_SLEEP");
    ESP.deepSleep(sleep_time_ns,WAKE_RF_DEFAULT);
  }else{
    //if we are going to wakeup and just record, disable wifi
    Serial.println("State: NO_RF_SLEEP");
     //ESP.deepSleep(sleep_time_ns);
      ESP.deepSleep(sleep_time_ns,WAKE_RF_DISABLED);
  }
  //ESP.deepSleep(sleep_time_ns);
