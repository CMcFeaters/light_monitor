...
*/
	...
	class my_wifi
	{
		public:	//publically accessible stuff
			my_wifi(char* ssid,char* pw, char* host,int port, IPAddress ip, IPAddress gateway);	//initiate, assigns all wifi data to private variables
			...
			void send_data(uint16_t* data,int size,int sleep_time, bool night_mode);	//sends the pointed to data to the location
			bool send_packet(WiFiClient client, String data, bool night_mode);//sends an individual packet
